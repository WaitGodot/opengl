...
int ffmpegTest()
{
	...
	if( avformat_open_input(&pFormatCtx, "test_hong_zhi_jian.avi", NULL, NULL) != 0 )
		return -1;

	if( avformat_find_stream_info(pFormatCtx, NULL ) < 0 )
		return -1;

	av_dump_format(pFormatCtx, -1, "test_hong_zhi_jian.avi", 0);
	videoStream = -1;
	...
	while( av_read_frame(pFormatCtx, &packet) >= 0 ) 
	{
		if( packet.stream_index == videoStream )
		{
			...
			if( frameFinished )
			{
				...
				//tx = saveToTexture(pFrameRGB,pCodecCtx->width, pCodecCtx->height);
				}
				sg_psp = Sprite::create(tx);
				if (tx != NULL)
				{
					sg_psp->setTexture(tx);
				}
